...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/trollocoin.conf are parsed in qt/bitcoin.cpp's main()
        ...
        {
            // First part of help message is specified in trollocoind / RPC client
            std::string strUsage = _("Trollocoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  trollocoind [options]                     " + "\n" +
                  "  trollocoind [options] <command> [params]  " + _("Send command to server or trollocoind") + "\n" +
                  "  trollocoind [options] help                " + _("List commands") + "\n" +
                  "  trollocoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "trollocoin:"))
                fCommandLine = true;
...
{
    ThreadSafeMessageBox(str, _("Trollocoin"), wxOK | wxMODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    ThreadSafeMessageBox(str, _("Trollocoin"), wxOK | wxICON_EXCLAMATION | wxMODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -conf=<file>          "   + _("Specify configuration file (default: trollocoin.conf)") + "\n" +
        "  -pid=<file>           "   + _("Specify pid file (default: trollocoind.pid)") + "\n" +
    ...
    strUsage += string() +
        _("\nSSL options: (see the Trollocoin Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-reservebalance")) // trollocoin: reserve balance amount
    {
    ...
    if (mapArgs.count("-checkpointkey")) // trollocoin: checkpoint master priv key
    {
    ...
    // Make sure only a single Trollocoin process is using the data directory.
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  Trollocoin is probably already running."), pszDataDir));
    ...
    //
    if (fDaemon)
        fprintf(stdout, "Trollocoin server starting\n");
    ...
    if (mapArgs.count("-onlynet")) {
        ...
        BOOST_FOREACH(std::string snet, mapMultiArgs["-onlynet"]) {
            ...
            if (net == NET_UNROUTABLE) {
                ThreadSafeMessageBox(_("Unknown network specified in -onlynet"), _("Trollocoin"), wxOK | wxMODAL);
                return false;
            }
            nets.insert(net);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        if (nLoadWalletRet == DB_CORRUPT)
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of Trollocoin") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart Trollocoin to complete") << "\n";
