...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "stop\n"
            "Stop Trollocoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Trollocoin server stopping";
}
...
// trollocoin: get network Gh/s estimate
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Trollocoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Trollocoin is downloading blocks...");

    if (pindexBest != NULL && pindexBest->nTime > POW_END_TIME)
        throw JSONRPCError(-10, "Trollocoin is currently on pure PoS state");
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: trollocoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer2(void* parg)
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use trollocoin(trollocoind)";
