...
// Copyright (c) 2014-2015 The Trollocoin developers
...
const string strMessageMagic = "Trollocoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // trollocoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // trollocoin: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // trollocoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// trollocoin: find block wanted by given orphan block
...
// trollocoin: miner's coin stake is rewarded based on coin age spent (coin-days)
...
// trollocoin: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (!fProofOfStake && pindexLast->nHeight >= 23 && pindexLast->nHeight < 120)
        ...
        // trollocoin: block stuck at 227
    if (!fProofOfStake && pindexLast->nHeight >= 277 && pindexLast->nHeight < 400)
        return bn227HashTarget.GetCompact();

    int64 nActualSpacing = pindexPrev->GetBlockTime() - pindexPrevPrev->GetBlockTime();

    // trollocoin: target change every block
    // trollocoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // trollocoin: should not enter safe mode for longer invalid chain
}
                                 ...
                                 bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // trollocoin: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // trollocoin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // trollocoin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // trollocoin: cleanup wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // trollocoin: track money supply and mint amount info
    ...
    // trollocoin: fees are not collected by miners as in bitcoin
    // trollocoin: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// trollocoin: total coin age spent in transaction, in the unit of coin-days.
...
// trollocoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // trollocoin: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // trollocoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // trollocoin: record proof-of-stake hash value
    ...
    // trollocoin: compute stake modifier
...
bool CBlock::CheckBlock(int64 nHeight) const
{
    ...
    // trollocoin: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // trollocoin: coinbase output should be empty if proof-of-stake block
    ...
    // trollocoin: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // trollocoin: check that the block satisfies synchronized checkpoint
    ...
    // trollocoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // trollocoin: check proof-of-stake
    ...
    // trollocoin: verify hash target and signature of coinstake tx
    if (pblock->IsProofOfStake())
    {
        ...
        if (!CheckProofOfStake(pblock->vtx[1], pblock->nBits, hashProofOfStake))
        {
            ...
            // trollocoin: ask for missing blocks
            if (pfrom)
    ...
    // trollocoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // trollocoin: getblocks may not obtain the ancestor block rejected
    ...
    // trollocoin: if responsible for sync-checkpoint send it
...
// trollocoin: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "Trollocoin", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Trollocoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // trollocoin: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // trollocoin: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // trollocoin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // trollocoin: should not enter safe mode for longer invalid chain
    // trollocoin: if sync-checkpoint is too old do not enter safe mode
    ...
    // trollocoin: if detected invalid checkpoint enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // trollocoin: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // trollocoin: record my external IP reported by peer
        ...
        // trollocoin: relay sync-checkpoint
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // trollocoin: best block is always the reference block to prevent
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // trollocoin: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // trollocoin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // trollocoin: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // trollocoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
