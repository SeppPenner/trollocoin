...
// trollocoin: optional setting to unlock wallet for block minting only;
...
// trollocoin: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // trollocoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            for (;;)
            {
                ...
                // trollocoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    if (!GetBoolArg("-avatar")) // trollocoin: not avatar mode
                    {
...
// trollocoin: create coin stake transaction
...
// trollocoin: check 'spent' consistency between wallet and txindex
// trollocoin: fix wallet spent state according to txindex
...
// trollocoin: disable transaction (only for coinstake)
